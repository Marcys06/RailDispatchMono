...
internal sealed class MyraGameplayView
{
        ...
        Action toggleRouteEdit)
    {
        ...
        root.Widgets.Add(BuildTopBar());
        Grid.SetRow(root.Widgets[^1], 0);
        Grid.SetColumnSpan(root.Widgets[^1], 3);

        root.Widgets.Add(BuildOperationsPanel());
        Grid.SetRow(root.Widgets[^1], 1);
        Grid.SetColumn(root.Widgets[^1], 0);

        root.Widgets.Add(BuildSelectionPanel());
        Grid.SetRow(root.Widgets[^1], 1);
        Grid.SetColumn(root.Widgets[^1], 1);

        root.Widgets.Add(BuildTrafficPanel());
        Grid.SetRow(root.Widgets[^1], 1);
        Grid.SetColumn(root.Widgets[^1], 2);

        root.Widgets.Add(BuildBottomBar());
        Grid.SetRow(root.Widgets[^1], 2);
        Grid.SetColumnSpan(root.Widgets[^1], 3);
    ...
    private Widget BuildTopBar()
    {
        ...
        bar.Widgets.Add(_clockLabel);
        bar.Widgets.Add(_dayLabel);
        bar.Widgets.Add(_simulationLabel);
        bar.Widgets.Add(_modeLabel);

        var speed1 = CreateButton("1x", 50);
        speed1.Click += (_, _) => _setSpeed(1f);
        var speed2 = CreateButton("2x", 50);
        speed2.Click += (_, _) => _setSpeed(2f);
        var speed5 = CreateButton("5x", 50);
        speed5.Click += (_, _) => _setSpeed(5f);
        bar.Widgets.Add(speed1);
        bar.Widgets.Add(speed2);
        bar.Widgets.Add(speed5);
    ...
    private Widget BuildOperationsPanel()
    {
        ...
        {
            Text = "Sterowanie ruchem, infrastrukturą i składem. HUD nie wykonuje decyzji za dyspozytora.",
            Wrap = true
        });

        _toolToggle = CreateButton("NARZĘDZIA  ▼", 260);
        _toolToggle.Click += (_, _) =>
        {
        ...
        {
            var button = CreateButton(item.Item1, 78);
            button.Click += (_, _) => _setSpeed(item.Item2);
            speeds.Widgets.Add(button);
        }
        panel.Widgets.Add(speeds);

        _toolHint = new Label
        {
            Text = "F6 manewry • F7 zmiana kierunku • C sprzęganie • X rozprzęganie • S trasa wagonu",
            Wrap = true
        };
    ...
    private Widget BuildSelectionPanel()
    {
        ...
        panel.Widgets.Add(_selectionLabel);
        panel.Widgets.Add(_selectionDetails);

        var debugHeader = new HorizontalStackPanel { Spacing = 6 };
        _debugToggle = CreateButton("DIAGNOSTYKA  ▼", 180);
        _debugToggle.Click += (_, _) =>
        {
            _debugExpanded = !_debugExpanded;
            _debugContent.Visible = _debugExpanded;
            _debugToggle.Content = new Label { Text = _debugExpanded ? "DIAGNOSTYKA  ▲" : "DIAGNOSTYKA  ▼" };
            RefreshDebug(TrainManager.Current);
        };
        debugHeader.Widgets.Add(_debugToggle);
        panel.Widgets.Add(debugHeader);
    ...
    private Widget BuildTrafficPanel()
    {
        ...
        _dispatcherLabel = new Label { Text = "Dyspozytor: —", Wrap = true };
        panel.Widgets.Add(_dispatcherLabel);

        panel.Widgets.Add(Header("POCIĄGI"));
        _trainList = CreateListGrid(335, 9);
        panel.Widgets.Add(new ScrollViewer { Height = 205, Content = _trainList });

        panel.Widgets.Add(Header("STACJE"));
        _stationSummary = new Label { Text = "—", Wrap = true };
        panel.Widgets.Add(_stationSummary);
        _stationList = CreateListGrid(335, 5);
        panel.Widgets.Add(new ScrollViewer { Height = 120, Content = _stationList });
    ...
    private Widget BuildBottomBar()
    {
        ...
        _wagonSummary = new Label { Text = "Brak wagonów.", Wrap = true };
        wagonPanel.Widgets.Add(_wagonSummary);
        _wagonList = CreateListGrid(590, 4);
        wagonPanel.Widgets.Add(new ScrollViewer { Height = 90, Content = _wagonList });
        panel.Widgets.Add(wagonPanel);

        var help = new VerticalStackPanel { Width = 600, Spacing = 2 };
        help.Widgets.Add(Header("STEROWANIE"));
        help.Widgets.Add(new Label
        {
            Text = "AUTOMAT: timetable + bloki + sygnały  |  RĘCZNIE: zwrotnice + sygnały + manewry  |  RADIOSTOP zatrzymuje automat",
            Wrap = true
        });
    ...
    private void RebuildLists(TrainManager manager)
    {
        ...
        foreach (var train in manager.Trains.Take(9))
        {
            ...
            string text = $"{marker}  {train.Id.ToString()[..6]}  {train.Speed * 3.6f:0.0} km/h{selected}";
            AddListButton(_trainList, text, () => _focusTrain(train));
        }
        ...
        {
            wagonTotal++;
            passengerTotal += wagon.PassengerCount;
            string delay = FormatDelay(wagon.ScheduleRuntime.DelaySeconds, wagon.Schedule != null);
            AddListButton(_wagonList,
                $"{wagon.ShortName}  {wagon.PassengerCount}/{wagon.PassengerCapacity}  opóźn. {delay}",
                () => _focusTrain(train),
                BuildWagonTooltip(wagon, manager.StationController));
    ...
    private void AddStationButton(Grid grid, Station station, int waiting, TrainManager manager)
    {
        ...
        string text = expanded ? BuildStationBreakdown(station, manager) : $"{station.Name}  •  oczekują: {waiting}";
        var button = new Button { Content = new Label { Text = text, Wrap = true }, HorizontalAlignment = HorizontalAlignment.Stretch };
        button.Click += (_, _) =>
        {
            if (!_expandedStations.Add(station.Id)) _expandedStations.Remove(station.Id);
            RebuildLists(manager);
        };
    ...
    private static string BuildStationBreakdown(Station station, TrainManager manager)
    {
        var groups = manager.StationController.Passengers.GetWaitingAt(station)
            ...
            .Select(g => new { Name = g.First().DestinationStation.Name, Count = g.Count() })
    ...
    private static string BuildWagonTooltip(Wagon wagon, StationController stations)
    {
        ...
        return "TRASA WAGONU\n" + string.Join("\n", ids.Select((id, i) =>
            $"{i + 1}. {stations.Stations.FirstOrDefault(s => s.Id == id)?.Name ?? id.ToString()[..8]}"));
    }
    ...
    private void RefreshDebug(TrainManager? manager)
    {
        ...
        if (_selectedTrain == null || manager == null)
        {
            _debugContent.Widgets.Add(new Label { Text = "Brak wybranego pociągu." });
            return;
        }

        var train = _selectedTrain;
        var composition = train.Composition;
        ...
        _debugContent.Widgets.Add(new Label { Text = $"RadioStop: {(train.IsRadioStopActive ? "AKTYWNY" : "nieaktywny")}", Wrap = true });
    }

    private static string BuildDispatcherStatus(Train train)
    {
        return RailwayDispatcher.Current?.GetStatus(train) ?? "DISPATCHER NIEDOSTĘPNY";
    }
    ...
    private static string BuildLocomotiveScheduleSummary(Train train)
    {
        ...
        string delay = FormatDelay(runtime.DelaySeconds, true);
        return $"Rozkład: {schedule.Name}\nPunkt: {runtime.CurrentPointIndex + 1}/{schedule.Points.Count} • opóźnienie: {delay}\nPlanowany odjazd: {FormatClock(point.DepartureSeconds)}";
    }

    private static string BuildModeText(TrainManager manager)
    {
        return $"TRYB: NORMALNY  •  POCIĄGI {manager.Trains.Count}  •  BLOKI {manager.BlockController?.Blocks.Count ?? 0}";
    }

    private void RebuildTools()
    {
        _toolContent.Widgets.Clear();
        AddTool("Tor prosty", TrackBuildMode.Straight);
        AddTool("Zakręt", TrackBuildMode.Curve);
        AddTool("Rozjazd", TrackBuildMode.Junction);
        AddTool("Semafor", TrackBuildMode.Signal);
        AddTool("Stacja", TrackBuildMode.Station);
        AddTool("Depot", TrackBuildMode.Depot);

        var route = CreateButton("TRASA WAGONU  [S]", 260);
        route.Click += (_, _) => _toggleRouteEdit();
        _toolContent.Widgets.Add(route);
    ...
    {
        var button = CreateButton(text, 260);
    ...
    private static Grid CreateListGrid(int width, int maxRows)
    {
        return new Grid { Width = width, HorizontalAlignment = HorizontalAlignment.Stretch, RowSpacing = 2 };
    }
    ...
    private static string FormatDelay(int seconds, bool hasSchedule)
    {
        ...
        int minutes = abs / 60;
        int rest = abs % 60;
        return minutes > 0 ? $"{sign}{minutes}m {rest:D2}s" : $"{sign}{rest}s";
    }
