...
internal sealed class MyraGameplayView
{
        ...
        Action<Train> releaseRoute, Action<Train> toggleAutomation)
    {
        ...
        // Inicjalizacja kontrolek przed użyciem
        ...
        _trains = ListGrid(350);
        _stations = ListGrid(350);
        _wagons = ListGrid(640);
        ...
        var center = BuildSelection();
        Grid.SetRow(center, 1);
        Grid.SetColumn(center, 1);
        root.Widgets.Add(center);

        var right = BuildTraffic();
        Grid.SetRow(right, 1);
    ...
    private Widget BuildTop()
    {
        ...
        bar.Widgets.Add(_clock);
        bar.Widgets.Add(_status);
        bar.Widgets.Add(_mode);
        bar.Widgets.Add(_infrastructure);

        foreach (var item in new[] { ("1x", 1f), ("2x", 2f), ("5x", 5f) })
        {
            var b = Button(item.Item1, 48);
            b.Click += (_, _) => _setSpeed(item.Item2);
            bar.Widgets.Add(b);
        }
    ...
    private Widget BuildTools()
    {
        ...
        var route = Button("TRASA WAGONU [S]", 285);
        route.Click += (_, _) => _toggleRouteEdit();
        panel.Widgets.Add(route);

        var f8 = Button("INFRASTRUKTURA TORU [F8]", 285);
        f8.Click += (_, _) => _setBuildMode(TrackBuildMode.None);
        panel.Widgets.Add(f8);

        panel.Widgets.Add(new Label
        {
            Text = "Zaznaczanie: LPM = nowa selekcja • Shift+LPM = dodaj • Ctrl+LPM = przełącz.",
            Wrap = true
        });
    ...
    private void RebuildTools()
    {
        ...
        _toolsToggle.Content = new Label
        {
            Text = _toolsOpen ? "NARZĘDZIA BUDOWY ▲" : "NARZĘDZIA BUDOWY ▼"
        };
    }

    private void AddTool(string text, TrackBuildMode mode)
    {
        var b = Button(text, 285);
        b.Click += (_, _) => _setBuildMode(mode);
        _tools.Widgets.Add(b);
    }
    ...
    private Widget BuildSelection()
    {
        ...
        panel.Widgets.Add(_selection);
        panel.Widgets.Add(_selectionDetails);

        var actions = new HorizontalStackPanel { Spacing = 4 };
        var force = Button("WYMUSZENIE", 120);
        force.Click += (_, _) => { if (_selectedTrain != null) _forceProceed(_selectedTrain); };

        var release = Button("ZWOLNIJ", 100);
        release.Click += (_, _) => { if (_selectedTrain != null) _releaseRoute(_selectedTrain); };

        var auto = Button("AUTO / RĘCZ", 125);
        auto.Click += (_, _) => { if (_selectedTrain != null) _toggleAutomation(_selectedTrain); };

        actions.Widgets.Add(force);
        actions.Widgets.Add(release);
        actions.Widgets.Add(auto);
        panel.Widgets.Add(actions);

        panel.Widgets.Add(Header("ZASADA RUCHU"));
        panel.Widgets.Add(new Label
        {
            Text = "FCFS: pierwszy zgłoszony pociąg wygrywa konflikt o blok. F6 omija arbitraż, ale nie zajętość bloku. RadioStop ma pierwszeństwo.",
            Wrap = true
        });
    ...
    private Widget BuildTraffic()
    {
        ...
        panel.Widgets.Add(_dispatcher);

        panel.Widgets.Add(Header("POCIĄGI"));
        panel.Widgets.Add(new ScrollViewer { Height = 250, Content = _trains });

        panel.Widgets.Add(Header("STACJE"));
        panel.Widgets.Add(new ScrollViewer { Height = 105, Content = _stations });
    ...
    private Widget BuildBottom()
    {
        ...
        wag.Widgets.Add(new ScrollViewer { Height = 85, Content = _wagons });
        panel.Widgets.Add(wag);

        var help = new VerticalStackPanel { Width = 650, Spacing = 2 };
        help.Widgets.Add(Header("SKRÓTY"));
        help.Widgets.Add(new Label
        {
            Text = "0 = zaznaczanie • 1–5/9 = budowa • F6 wymuś przejazd • F7 kierunek • F8 infrastruktura • F9 rozkład • F10 diagnostyka • F11 linie kolejowe • C/X sprzęganie/rozprzęganie.",
            Wrap = true
        });
    ...
    {
        _trains.Widgets.Clear();
        _trains.RowsProportions.Clear();
        _stations.Widgets.Clear();
        _stations.RowsProportions.Clear();
        _wagons.Widgets.Clear();
        _wagons.RowsProportions.Clear();
        ...
        foreach (var train in manager.Trains)
        {
            ...
            {
                // POPRAWKA: Użycie ?? 0 do konwersji int? na int
                int delaySeconds = wagon.ScheduleRuntime?.DelaySeconds ?? 0;
                string delay = FormatDelay(delaySeconds);
                Add(_wagons, $"{wagon.ShortName} • {wagon.PassengerCount}/{wagon.PassengerCapacity} • {delay}", () => _focusTrain(train));

                if (_wagons.Widgets.Count >= 6)
                    break;
            }
    ...
    private void RefreshSelection()
    {
        ...
        {
            _selection.Text = "Nie wybrano pociągu.";
            _selectionDetails.Text = "Wybierz skład z panelu RUCH.";
            return;
        }
    ...
    {
        var s = train.LocomotiveSchedule;
        var r = train.LocomotiveScheduleRuntime;

        if (s == null || !s.Enabled)
            return "RĘCZ / wyłączony";

        if (r == null || s.Points.Count == 0)
            return s.Name;
    ...
    private static Grid ListGrid(int width) => new()
    {
        Width = width,
    ...
    {
        var b = Button(text, grid.Width - 5);
        b.Click += (_, _) => action();
        Grid.SetRow(b, grid.Widgets.Count);
