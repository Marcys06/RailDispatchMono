...
internal sealed class MyraDepotView
{
    ...
    public MyraDepotView(DepotScreen screen)
    {
        ...
        {
            var button = new Button { Content = new Label { Text = $"{definition.DisplayName} • {definition.MaxSpeedKmh:0} km/h • {definition.MassTons:0} t" }, Width = 370 };
        ...
        {
            var button = new Button { Content = new Label { Text = $"{definition.DisplayName} • {definition.MassTons:0} t • {definition.MaxSpeedKmh:0} km/h" }, Width = 370 };
    ...
    public void Refresh()
    {
        ...
        for (int i = 0; i < _screen.Composition.Vehicles.Count; i++)
        {
            ...
            row.Widgets.Add(new Label { Text = $"{i + 1}. {(vehicle is Locomotive ? loco?.DisplayName ?? "Lokomotywa" : "Wagon")}", Width = 300 });
            if (vehicle is Wagon)
            {
