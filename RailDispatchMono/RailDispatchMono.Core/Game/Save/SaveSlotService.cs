...
public static class SaveSlotService
{
    ...
    {
        if (!Directory.Exists(SaveSlotContext.RootDirectory))
        ...
        foreach (string directory in Directory.GetDirectories(SaveSlotContext.RootDirectory))
        {
            ...
            {
                // Broken slots are deliberately omitted from the normal list.
            }
        }
    ...
    public static string CreateSlot()
    {
        ...
        // ':' is illegal in Windows directory names. Keep the requested display
        // name in metadata while using a filesystem-safe directory name.
        ...
        while (Directory.Exists(directory))
            directory = Path.Combine(SaveSlotContext.RootDirectory, directoryName + "-" + suffix++);

        Directory.CreateDirectory(directory);
        SaveSlotContext.SetActiveSlot(directory);
        WriteMetadata(new SaveSlotMetadata
        {
            Name = displayName,
            CreatedAt = now,
            LastSavedAt = now
    ...
    public static void Activate(string directory)
    {
        ...
        if (!File.Exists(Path.Combine(directory, "metadata.json")))
            throw new InvalidDataException("metadata.json is missing.");
        SaveSlotContext.SetActiveSlot(directory);
    }
    ...
    {
        string directory = SaveSlotContext.ActiveSlotDirectory
            ?? throw new InvalidOperationException("No active save slot.");
