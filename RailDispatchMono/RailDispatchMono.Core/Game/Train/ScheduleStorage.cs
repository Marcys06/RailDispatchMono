...
/// <summary>Persists locomotive and wagon schedules into the active save slot.</summary>
public static class ScheduleStorage
{
    ...
    public static string FilePath => SaveSlotContext.ActiveSlotDirectory != null ? Path.Combine(SaveSlotContext.ActiveSlotDirectory, "schedules.json") : Path.Combine(DirectoryPath, "schedules.json");
    ...
    public static TrainSchedule? Load(Guid trainId) => LoadDocument().Schedules.FirstOrDefault(x => x.TrainId == trainId);
    ...
    private static ScheduleDocument LoadDocument()
    {
        ...
        try { return JsonSerializer.Deserialize<ScheduleDocument>(File.ReadAllText(FilePath)) ?? new ScheduleDocument(); }
        catch (JsonException ex) { throw new InvalidDataException("schedules.json is invalid.", ex); }
    }
}
