...
public sealed class PassengerManager
{
    ...
    public int BoardPassengers(TrainClass train, Station station)
    {
        ...
        foreach (var passenger in waiting)
        {
            ...
            foreach (var vehicle in train.Composition.Vehicles)
                if (vehicle is Wagon wagon && wagon.CanAcceptPassenger(passenger)) { selectedWagon = wagon; break; }
            if (selectedWagon == null) break;
            if (selectedWagon.TryBoard(passenger, train.Id)) boarded++;
        }
        return boarded;
    }
    ...
    public int AlightPassengers(TrainClass train, Station station)
    {
        int alighted = 0;
        foreach (var vehicle in train.Composition.Vehicles)
            if (vehicle is Wagon wagon) alighted += wagon.TryAlightAt(station, train.Id);
        return alighted;
    }
