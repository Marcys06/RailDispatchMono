...
public sealed class RailwayLine
{
    ...
    public RailwayLine(string name) => Name = string.IsNullOrWhiteSpace(name) ? "Nowy szlak" : name.Trim();
...
public sealed class RailwayLineManager
{
    ...
    public RailwayLineManager(GameMap map) { _map = map; Current = this; }
    public RailwayLine Create(string name, IEnumerable<MapPosition> positions)
    {
        var line = new RailwayLine(name);
        foreach (var position in positions.Distinct()) if (_map.HasTrack(position)) line.Add(position);
        _lines.Add(line); return line;
    }
    public bool Delete(Guid id) { int index = _lines.FindIndex(x => x.Id == id); if (index < 0) return false; _lines.RemoveAt(index); return true; }
    ...
    public int ApplyInfrastructure(RailwayLine line, TrackType type, LineClass lineClass, TractionSystem traction)
    {
        ...
        {
            var track = _map.GetTrackAt(position); if (track == null) continue;
            track.SetInfrastructure(type, lineClass, traction); count++;
        }
        return count;
    }
    public static IReadOnlyList<MapPosition> CollectConnectedTracks(GameMap map, MapPosition start)
    {
        ...
        var result = new List<MapPosition>(); var visited = new HashSet<MapPosition>(); var queue = new Queue<MapPosition>();
        queue.Enqueue(start); visited.Add(start);
        while (queue.Count > 0)
        {
            var position = queue.Dequeue(); result.Add(position); var track = map.GetTrackAt(position); if (track == null) continue;
            foreach (var direction in track.GetAvailableDirections())
            {
                var next = Next(position, direction); if (visited.Contains(next) || !map.HasTrack(next)) continue;
