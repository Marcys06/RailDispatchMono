...
public sealed class WagonRouteMenu
{
    ...
    {
        _wagon = wagon; _stations = stations; _position = screenPosition + new Vector2(12f, 12f);
        _previousMouse = Mouse.GetState(); ClampToViewport();
    }
    public void Close() { _wagon = null; _stations = null; }
    ...
    public void Update(MouseState mouse)
    {
        ...
        {
            if (ButtonRect(0, 0, 240, 34).Contains(mouse.Position)) { Close(); _previousMouse = mouse; return; }
            int y = 44;
            foreach (var stationId in _wagon.Route.StationIds.ToList())
            {
                if (ButtonRect(0, y, 240, 30).Contains(mouse.Position)) { if (_wagon.Route.RemoveStation(stationId)) Changed(); break; }
                y += 34;
            }
            y += 6;
            foreach (var station in _stations.Stations)
            {
                ...
                if (ButtonRect(0, y, 240, 30).Contains(mouse.Position)) { _wagon.Route.AddStation(station.Id); Changed(); break; }
                y += 34;
            }
        ...
        if (mouse.RightButton == ButtonState.Pressed && _previousMouse.RightButton == ButtonState.Released) Close();
        if (mouse.X < _position.X - 10 || mouse.X > _position.X + 250 || mouse.Y < _position.Y - 10 || mouse.Y > _position.Y + GetHeight() + 10)
            if (mouse.LeftButton == ButtonState.Pressed) Close();
    ...
    public void Draw(SpriteBatch spriteBatch)
    {
        ...
        ClampToViewport(); float width = 250f, height = GetHeight();
        ...
        {
            if (_wagon.Route.ServesStation(station.Id)) continue;
            DrawButton(spriteBatch, new Rectangle((int)_position.X + 5, (int)_position.Y + y, 240, 30), $"+ {station.Name}", false); y += 34;
        }
    ...
    private Rectangle ButtonRect(int x, int y, int width, int height) => new((int)_position.X + 5 + x, (int)_position.Y + y, width, height);
    private void DrawButton(SpriteBatch batch, Rectangle rect, string text, bool selected)
    {
        ...
        float scale = 0.55f; Vector2 size = _font!.MeasureString(text) * scale;
    ...
    private void DrawString(SpriteBatch batch, string text, int x, int y, Color color, float scale) => batch.DrawString(_font!, text, _position + new Vector2(x, y), color, 0f, Vector2.Zero, scale, SpriteEffects.None, 0f);
