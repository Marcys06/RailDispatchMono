...
/// <summary>
/// Lightweight screen-space editor for one wagon's station route.
/// It intentionally uses the same generated pixel/font UI as the rest of the game.
/// </summary>
public sealed class WagonRouteMenu
{
    ...
    {
        if (!IsOpen || _wagon == null || _stations == null)
            return;
        ...
        if (mouse.LeftButton == ButtonState.Pressed && _previousMouse.LeftButton == ButtonState.Released)
        {
            ...
            foreach (var stationId in _wagon.Route.StationIds.ToList())
            {
                ...
                {
                    _wagon.Route.RemoveStation(stationId);
                    break;
                }
                y += 34;
            }
            ...
            {
                if (_wagon.Route.ServesStation(station.Id))
                    continue;
                if (ButtonRect(0, y, 240, 30).Contains(mouse.Position))
                {
        ...
        if (mouse.X < _position.X - 10 || mouse.X > _position.X + 250 ||
            mouse.Y < _position.Y - 10 || mouse.Y > _position.Y + GetHeight() + 10)
        {
    ...
    {
        if (!IsOpen || _wagon == null || _stations == null || _font == null || _pixel == null)
            return;
    ...
    private float GetHeight()
    {
        ...
        int rows = _wagon.Route.StationIds.Count + addable;
        return 82f + rows * 34f + 36f;
    }
