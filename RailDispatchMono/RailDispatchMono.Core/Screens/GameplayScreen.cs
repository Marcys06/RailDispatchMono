...
public sealed class GameplayScreen : GameScreen
{
    ...
    public override void Update(GameTime gameTime, bool otherScreenHasFocus, bool coveredByOtherScreen)
    {
        ...
        _map.Maintenance.Update(deltaTime, GetOccupiedTrackPositions());
    ...
    private IEnumerable<MapPosition> GetOccupiedTrackPositions()
    {
        foreach (Train train in _trainManager.Trains)
        {
            int x = (int)MathF.Floor(train.Position.X);
            int y = (int)MathF.Floor(train.Position.Y);
            var position = new MapPosition(x, y);
            if (_map.HasTrack(position)) yield return position;
        }
    }
